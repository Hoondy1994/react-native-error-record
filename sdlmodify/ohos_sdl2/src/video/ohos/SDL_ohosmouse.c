...
void OHOS_OnMouseAxis(SDL_Window *window, OHOSMouseAxisData *mouseAxisData)
{
    if (!window || !mouseAxisData) {
        SDL_LogError(SDL_LOG_CATEGORY_INPUT, "Invalid window or mouseAxisData");
        return;
    }
    SDL_SendMouseWheel(window, 0, mouseAxisData->wheelX, mouseAxisData->wheelY, SDL_MOUSEWHEEL_NORMAL);
}
