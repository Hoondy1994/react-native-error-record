...
const FastImageDemo = () => {
  ...
  // 本地图片（asset://）：按你们工程的方式加载，排除网络影响
  const ASSET_URI = "asset://test1.png";
  const images = [ASSET_URI, ASSET_URI, ASSET_URI, ASSET_URI, ASSET_URI];
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.content}>
        <Text style={styles.title}>折叠屏 FastImage 延迟测试</Text>
            ...
            <View key={index} style={styles.imageBox}>
              ...
              <FastImage
                ...
                onProgress={(e: OnProgressEvent) => {
                  const total = e?.nativeEvent?.total ?? 0;
                  const loaded = e?.nativeEvent?.loaded ?? 0;
                  const progress = total > 0 ? ((loaded / total) * 100).toFixed(1) : "0.0";
                  console.log(
                    `图片${index + 1} onProgress ${progress}% (${loaded}/${total})`
                  );
                }}
        ...
        <View style={styles.analysisBox}>
          ...
          {loadedCount === 0 && (
            <Text style={styles.analysisText}>
              • 等待图片开始加载...{"\n"}
              • 本地图片一般不会卡在网络阶段；如果长时间没加载，优先看解码/渲染链路
            </Text>
          )}
